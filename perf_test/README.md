...
Results:
  Success rate:  34.04%
  Total requests: 100000
  Total duration:      8m50.001049s
  Actual rate:   200.00 req/s
  Throughput:    64.23 req/s
Latencies:
  Min:  47.15ms
  Mean: 23.364261518s
  50th: 21.049165196s
  ...
  Max:  30.0361064s
  Bytes (In/Out): 18 / 65
  Status codes: map[0:65723 200:34040 400:237]
```
- Чтение
```
Results:
...
Latencies:
  ...
  Max:  30.0003954s
  Bytes (In/Out): 149 / 0
  Status codes: map[0:1 200:34061 404:65938]
```

Из новых результатов видно, что добавленные индексы ускорили работу сервиса.
- Процент успешных ответов вырос на 10%.
- Пропускная способность увеличилась на 40% (особенно заметно на операциях записи).
- Уменьшилось количество ответов со статусом 0.
- В чтении один запрос выполнялся больше 30с (который вызвал статус код 0), но будем считать его статистическим выбросом.
- Результаты задержек при записи немного улучшились.
Это видно по медиане и среднему значению, которые уменьшились на 1-2 с.
- Результаты чтения ухудшились, но это, скорее всего, связано со сменой железа и сети между тестами. (Чуть позже попробую провести тесты чтения на первоначальном железе).
