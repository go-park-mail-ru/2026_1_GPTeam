...
func TestValidateBudget(t *testing.T) {
	...
	t.Run("All correct", func(t *testing.T) {
		...
		errs := validators.ValidateBudget(body, currencies, allowedCategories)
...
func TestValidateTransactionCategory(t *testing.T) {
	...
	testCases := []struct {
		...
		{"Invalid category", "entertainment", validators.CategoryNotAllowedError},
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			t.Parallel()
			err := validators.ValidateCategory(testCase.Category, allowed)
			require.ErrorIs(t, err, testCase.err)
		})
	}
}
